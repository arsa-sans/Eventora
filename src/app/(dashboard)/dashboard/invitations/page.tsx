  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-serif text-foreground">Undangan Saya</h2>
          <p className="text-text-secondary mt-1">Kelola semua undangan digital Anda.</p>
        </div>
      ...
      {invitations.length === 0 ? (
        ...
        <div className="grid gap-4">
          {invitations.map((inv) => {
            ...
            return (
              <div key={inv.id} className="bg-white rounded-2xl border border-border shadow-sm p-6 flex flex-col md:flex-row md:items-center gap-4">
                {/* Info */}
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted mb-2 flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 text-[11px] text-muted">
                    <span className="bg-gray-100 px-2 py-0.5 rounded font-mono truncate max-w-[200px]">
                      /{inv.slug}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => copyLink(inv.slug)}
                    className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-semibold bg-gray-100 hover:bg-gray-200 transition-colors">
                    {copiedSlug === inv.slug ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
                    {copiedSlug === inv.slug ? "Tersalin!" : "Salin Link"}
                  </button>

                  <Link href={`/dashboard/invitations/${inv.id}/edit`}>
                    <button className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-semibold bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors">
                      <Pencil className="w-3.5 h-3.5" /> Edit
                    </button>
                  </Link>

                  <Link href={`/${inv.slug}`} target="_blank">
                    <button className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-semibold bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
                  ...
                  {inv.status === "draft" && (
                    <button onClick={() => handleActivate(inv)} disabled={checkingOut === inv.id}
                      className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-semibold bg-primary text-white hover:bg-primary-dark transition-colors disabled:opacity-50">
                      {checkingOut === inv.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CreditCard className="w-3.5 h-3.5" />}
                      Aktifkan
                    </button>
                  )}

                  <button onClick={() => handleDelete(inv.id)} disabled={deleting === inv.id}
                    className="flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-semibold text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50">
